...
def vis_detections_label_only(im, class_name, dets, thresh=0.8):
    ...
    for i in range(np.minimum(10, dets.shape[0])):
        ...
        score = dets[i, -1]
        if score > thresh:
            cv2.rectangle(im, bbox[0:2], bbox[2:4], (220, 170, 143), 2)
            text_size = cv2.getTextSize('%s' % (class_name), cv2.FONT_HERSHEY_PLAIN, 0.8, 1)
            point = (bbox[0] + text_size[0][0], bbox[1] + text_size[0][1] + text_size[1])
            cv2.rectangle(im, bbox[0:2], point, (220, 170, 143), -1)
            cv2.putText(im, '%s' % (class_name), (bbox[0], bbox[1] + 10), cv2.FONT_HERSHEY_PLAIN,
